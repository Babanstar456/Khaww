...
app.get('/api/customers', async (req, res) => {
  try {
    ...
    const customers = rows.map(customer => ({
      ...customer,
      address: parseJsonSafe(customer.address, []),
    }));

    res.json({ success: true, data: { customers } });
  } catch (err) {
    handleError(res, err, 'fetching customers');
  }
});
...
app.listen(PORT, 'localhost', () => {
  ...
  console.log('âœ… Endpoints: /api/restaurants, /api/menu, /api/categories, /api/customers, /api/orders, /health');
  console.log('ðŸ›°ï¸ Socket.IO events: joinRestaurant, joinCustomer, newOrder, orderPlaced, orderStatusUpdated');
